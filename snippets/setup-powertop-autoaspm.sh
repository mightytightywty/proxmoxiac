...
cat <<EOF
##############################################################################
# Install & Configure Powertop and AutoASPM (tested on Proxmox)
# Usage: setup-powertop-autoaspm.sh
...
# Update & Install pciutils and powertop
echo "Installing required packages..."
apt update -qq && apt install -y pciutils python3 powertop

# Create a local copy of autoaspm.py
cat <<'EOF' > "$INSTALL_DIR/autoaspm.py"
#!/usr/bin/env python3
...
chmod +x "$INSTALL_DIR/autoaspm.py"

# Create combined systemd service for PowerTOP and AutoASPM
cat <<EOF > /etc/systemd/system/powertop-autoaspm.service
[Unit]
Description=PowerTOP Auto Tuner and AutoASPM
...
ExecStart=/usr/bin/python3 $INSTALL_DIR/autoaspm.py
RemainAfterExit=yes
...
systemctl enable powertop-autoaspm.service
systemctl restart powertop-autoaspm.service

cat <<EOF
----------------------- POWERTOP AUTOASPM INSTALL & CONFIGURE COMPLETE --------------------------

###############################
# Power Saving Cheat Sheet
###############################

# Troubleshooting: Run powertop manually > Tab over to "tunables" > Press enter to toggle each item. Bad/Good refers to power savings, not stability.
# Changes/Exclusions will be lost on reboot unless also added to /etc/systemd/system/powertop-autoaspm.service
powertop

# Check ASPM support
lspci -vv | grep ASPM | grep -i enabled

# Disable if system hangs
systemctl disable powertop-autoaspm.service
